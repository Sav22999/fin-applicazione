...
class DatabaseHandler(context: Context) :
    ...
    fun getMistakesStatistics(): ArrayList<StatisticsModel> {
        ...
        val statisticsList = ArrayList<StatisticsModel>()
        val query =
            "SELECT DISTINCT `${COLUMN_QUESTION_ID_STATISTICS}` FROM `${TABLE_NAME_STATISTICS}` WHERE `${COLUMN_TYPE_STATISTICS}` = '1' AND (NOT `${COLUMN_CORRECT_ANSWER_STATISTICS}` = `${COLUMN_USER_ANSWER_STATISTICS}`) ORDER BY `${COLUMN_DATETIME_STATISTICS}` DESC"
